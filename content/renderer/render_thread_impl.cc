...
#include "third_party/blink/public/common/fingerprint/fingerprint_mojo_convert.h"
...
void RenderThreadImpl::InitializeRenderer(
    ...
    uint64_t trace_id,
    blink::mojom::FingerprintPtr mojo_fingerprint) {
  ...
  if (mojo_fingerprint) {
    fingerprint = blink::fp::FingerprintFromMojo(mojo_fingerprint);
  }
  ...
  if(base::SingletonFingerprint::GetInit(t_singletonFingerprint) && t_fingerprint.timeZone.type>1){
    icu::TimeZone::adoptDefault(
            icu::TimeZone::createTimeZone(icu::UnicodeString::fromUTF8(t_fingerprint.timeZone.gmt)));
    // 通知 V8 刷新 DateCache，使 getTimezoneOffset() 和 Intl 时区同步
  ...
  // CrossGuard: 设置 ICU 默认 locale，使 Intl API 返回正确的语言
  if(base::SingletonFingerprint::GetInit(t_singletonFingerprint) && t_fingerprint.language.type>1){
    std::string lang = t_fingerprint.language.interfaceLanguage;
    if(!lang.empty()){
