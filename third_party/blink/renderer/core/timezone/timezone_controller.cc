...
// CrossGuard: 如果 SingletonFingerprint 设置了自定义时区，返回该时区 ID
String GetFingerprintTimezoneOverride() {
  if (!base::SingletonFingerprint::HasInstance())
    return String();
  base::SingletonFingerprint* fp = base::SingletonFingerprint::ForCurrentProcess();
  if (fp && base::SingletonFingerprint::GetInit(fp)) {
    blink::fp::Fingerprint fingerprint = fp->GetFingerprint();
    if (fingerprint.timeZone.type > 1 && !fingerprint.timeZone.gmt.empty()) {
      return String(fingerprint.timeZone.gmt.c_str());
    }
  }
  return String();
}
...
TimeZoneController::TimeZoneController() {
  ...
  // CrossGuard: 如果 SingletonFingerprint 已设置自定义时区，立即应用
  String fp_tz = GetFingerprintTimezoneOverride();
  if (!fp_tz.empty()) {
    SetIcuTimeZoneAndNotifyV8(fp_tz);
  }
}
...
void TimeZoneController::OnTimeZoneChange(const String& timezone_id) {
  ...
  // CrossGuard: 如果 SingletonFingerprint 设置了自定义时区，使用它
  String fp_tz = GetFingerprintTimezoneOverride();
  if (!fp_tz.empty()) {
    if (!HasTimeZoneOverride()) {
      SetIcuTimeZoneAndNotifyV8(fp_tz);
