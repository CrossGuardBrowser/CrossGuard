...
void Canvas2DRecorderContext::FillImpl(SkPathFillType winding_rule) {
  CanvasRenderingContext* ctx = GetCanvasRenderingContext();
  bool flag = ctx ? blink::fp::fpCanvas(ctx, GetState(), *this, true) : false;
  if (identifiability_study_helper_.ShouldUpdateBuilder()) [[unlikely]] {
  ...
  if (ctx) ctx->setFpCanvasTrue(flag);
}
...
void Canvas2DRecorderContext::FillPathImpl(Path2D* dom_path,
  ...
  CanvasRenderingContext* ctx = GetCanvasRenderingContext();
  bool flag = ctx ? blink::fp::fpCanvas(ctx, GetState(), *dom_path, true) : false;
  if (identifiability_study_helper_.ShouldUpdateBuilder()) [[unlikely]] {
    identifiability_study_helper_.UpdateBuilder(
...
void Canvas2DRecorderContext::stroke() {
  CanvasRenderingContext* ctx = GetCanvasRenderingContext();
  bool flag = ctx ? blink::fp::fpCanvas(ctx, GetState(), *this, false) : false;
  if (identifiability_study_helper_.ShouldUpdateBuilder()) [[unlikely]] {
...
void Canvas2DRecorderContext::stroke(Path2D* dom_path) {
  CanvasRenderingContext* ctx = GetCanvasRenderingContext();
  bool flag = ctx ? blink::fp::fpCanvas(ctx, GetState(), *dom_path, false) : false;
  if (identifiability_study_helper_.ShouldUpdateBuilder()) [[unlikely]] {
    identifiability_study_helper_.UpdateBuilder(
...
void Canvas2DRecorderContext::fillRect(double x,
  ...
  CanvasRenderingContext* ctx = GetCanvasRenderingContext();
  bool flag = ctx ? blink::fp::fpCanvas(ctx, GetState(), true) : false;
  double orig_x = x, orig_y = y, orig_w = width, orig_h = height;
  if (identifiability_study_helper_.ShouldUpdateBuilder()) [[unlikely]] {
  ...
  if (ctx) {
    ctx->setFpCanvasTrue(flag);
    blink::fp::fpFullCoverage(ctx, orig_x, orig_y, orig_w, orig_h, 0, 0, Width(), Height());
  }
}
...
void Canvas2DRecorderContext::drawImage(CanvasImageSource* image_source,
                                        ...
                                        double dh,
                                        ExceptionState& exception_state) {
  double orig_dx = dx, orig_dy = dy, orig_dw = dw, orig_dh = dh;
  if (!GetOrCreatePaintCanvas()) {
    return;
  }
  ...
  CanvasRenderingContext* ctx = GetCanvasRenderingContext();
  if (ctx) {
    blink::fp::fpFullCoverage(ctx, orig_dx, orig_dy, orig_dw, orig_dh, 0, 0, Width(), Height());
  }
}
