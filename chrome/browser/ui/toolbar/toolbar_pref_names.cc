...
void RegisterProfilePrefs(user_prefs::PrefRegistrySyncable* registry) {
  ...
  // CrossGuard AI chat — 默认 pin 到工具栏。
  const std::optional<std::string>& crossguard_chat_action =
      actions::ActionIdMap::ActionIdToString(kActionSidePanelShowCrossGuardChat);
  if (crossguard_chat_action.has_value()) {
    default_pinned_actions.Append(crossguard_chat_action.value());
  }
