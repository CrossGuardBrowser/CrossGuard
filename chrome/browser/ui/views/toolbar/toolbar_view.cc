...
void ToolbarView::Init() {
  ...
  // CrossGuard AI chat 按钮(头像左侧,点击切换 side panel)
  crossguard_chat_button_ = container_view_->AddChildView(
      std::make_unique<ToolbarButton>(base::BindRepeating(
          [](Browser* browser) {
            auto* coordinator = browser->GetFeatures().side_panel_coordinator();
            if (!coordinator) return;
            auto key = SidePanelEntry::Key(SidePanelEntry::Id::kCrossGuardChat);
            if (coordinator->IsSidePanelEntryShowing(key)) {
              coordinator->Close();
            } else {
              coordinator->Show(key, SidePanelOpenTrigger::kToolbarButton);
            }
          },
          browser_)));
  crossguard_chat_button_->SetVectorIcon(vector_icons::kChatSparkIcon);
  crossguard_chat_button_->SetTooltipText(u"AI 对话");
  crossguard_chat_button_->SetHorizontalAlignment(gfx::ALIGN_CENTER);
  crossguard_chat_button_->SetAccessibleName(u"CrossGuard AI");
